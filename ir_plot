...
# ir plot in terminal
# taken and adapted from: https://github.com/radi0sus/orca_ir
...
# use vibspectrum file from xtb
parser.add_argument('-x','--xtb',
    default=0, action='store_true',
    help='use vibspectrum file from xtb')
        ...
        if xtb:
            ...
            for line in input_file:
                if line.startswith("#") or line.startswith('$'):
                    continue
                line = ','.join(line.replace('a','').split()).split(',')
                modelist.append(int(line[0]))
                freqlist.append(float(line[1]))
                intenslist.append(float(line[2]))
        else:
            for line in input_file:
                #start exctract text 
